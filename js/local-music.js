...
function initLocalMusicUpload() {
    ...
    if (uploadInput) {
        ...
        Logger.debug('本地音乐上传初始化成功');
    } else {
        Logger.debug('未找到 localMusicUpload 元素');
    }
}
...
async function handleLocalMusicUpload(event) {
    ...
    Logger.info('开始导入音乐:', files.length);
    ...
    for (let i = 0; i < files.length; i++) {
        ...
        if (!file.type.startsWith('audio/')) {
            Logger.debug('跳过非音频文件:', file.name);
            continue;
        }
        ...
        try {
            ...
            Logger.debug('已保存到 IndexedDB:', fileName);
        } catch (err) {
            Logger.error('保存到 IndexedDB 失败:', err);
        }
        
        Logger.debug('已添加:', fileName);
    }
    ...
    Logger.info('导入完成，共导入:', importedCount, '首');
}
...
async function removeLocalMusic(songId) {
    ...
    try {
        ...
        Logger.error('从 IndexedDB 删除文件失败:', err);
    }
...
async function clearLocalMusic() {
    ...
    try {
        ...
        Logger.error('清空 IndexedDB 失败:', err);
    }
...
async function loadLocalMusicListFromIndexedDB() {
    try {
        ...
        if (savedFiles && savedFiles.length > 0) {
            Logger.info('从 IndexedDB 恢复', savedFiles.length, '首本地音乐');
            ...
            for (const fileData of savedFiles) {
                try {
                    ...
                    Logger.error('恢复音乐失败:', fileData.name, err);
                }
            }
            ...
            Logger.info('恢复完成，共恢复', restoredCount, '首本地音乐');
        }
    } catch (err) {
        Logger.error('从 IndexedDB 恢复音乐失败:', err);
    }
}
...
window.addEventListener('beforeunload', function() {
    ...
    Logger.info('页面卸载，已释放所有 Blob URL');
});
